...
function formatDate(str: string): string {
  if (!str) return "—";
  // sailingDate vine ca "02/05/2026"
...
function CruiseCard({ fare, onSelect }: { fare: Fare; onSelect: () => void }) {
  ...
  const shipInfo     = getShipInfo(fare.shipCd);
      ...
      <div className="flex-1 flex flex-col md:flex-row">
        ...
        <div className="flex md:flex-col items-center md:items-end justify-between md:justify-center gap-3 px-4 md:px-5 py-3 md:py-5 md:min-w-[150px] border-t md:border-t-0 md:border-l border-gray-100">
          <div className="text-right">
            ...
            <p className="text-xs text-gray-400 mt-1">taxe incluse</p>
          </div>
          <button
...
function Results() {
  ...
  const toggleShip = (code: string) =>
    setFilterShips((prev) => prev.includes(code) ? prev.filter((c) => c !== code) : [...prev, code]);

  const togglePort = (code: string) =>
    setFilterPorts((prev) => prev.includes(code) ? prev.filter((c) => c !== code) : [...prev, code]);

  const toggleCabin = (type: CabinType) =>
    setFilterCabins((prev) => prev.includes(type) ? prev.filter((c) => c !== type) : [...prev, type]);
  ...
  const resetFilters = () => {
    ...
    setFilterCabins([]);
    setFilterNights("");
    setPriceSlider(filterMaxPrice);
  };

  const hasFilters = filterShips.length > 0 || filterPorts.length > 0 ||
    filterCabins.length > 0 || filterNights !== "" || priceSlider < filterMaxPrice;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Top bar */}
      <div className="bg-blue-900 text-white px-4 py-3 sticky top-0 z-20 shadow-lg">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-blue-200">
              {loading ? "Se caută..." : `${filteredFares.length} croaziere`}
            </span>
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
              className="text-sm bg-blue-800 border border-blue-700 text-white rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-400"
            >
              {SORT_OPTIONS.map((o) => (
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 flex gap-6">
        ...
        <aside className="hidden lg:block w-60 flex-shrink-0">
          <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-16">
                ...
                {nightsOptions.map((o) => (
                  <label key={o.value} className="flex items-center gap-2 cursor-pointer group">
                    <input
                      type="radio"
                      name="nights"
                      checked={filterNights === o.value}
                      onChange={() => setFilterNights(filterNights === o.value ? "" : o.value)}
              ...
              <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto">
                {shipOptions.map((s) => (
                  <label key={s.code} className="flex items-center gap-2 cursor-pointer group">
                    <input
                      type="checkbox"
                      checked={filterShips.includes(s.code)}
                      onChange={() => toggleShip(s.code)}
              ...
              <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto">
                {portOptions.map((p) => (
                  <label key={p.code} className="flex items-center gap-2 cursor-pointer group">
                    <input
                      type="checkbox"
                      checked={filterPorts.includes(p.code)}
                      onChange={() => togglePort(p.code)}
                      className="accent-blue-600 rounded"
                    />
            ...
            <FilterSection title="Tip cabină">
              {(["interior", "exterior", "balcon", "suite"] as CabinType[]).map((type) => {
                const labels = { interior: "🛏 Interioară", exterior: "🪟 Exterioară", balcon: "🌊 Balcon", suite: "⭐ Suite & YC" };
                ...
                return (
                  <label key={type} className="flex items-center gap-2 cursor-pointer group mb-1.5">
                    <input
                      type="checkbox"
                      checked={filterCabins.includes(type)}
                      onChange={() => toggleCabin(type)}
                      className="accent-blue-600 rounded"
                    />
          ...
          {!loading && !error && filteredFares.length === 0 && (
            <div className="text-center py-32">
              ...
              <p className="text-gray-700 font-semibold text-lg mb-2">
                {allFares.length === 0
                  ? "Nu am găsit croaziere pentru perioada selectată"
                  : "Niciun rezultat pentru filtrele selectate"}
              </p>
              <p className="text-gray-400 text-sm mb-6">
                {allFares.length > 0
                  ? "Încearcă să modifici sau să resetezi filtrele"
                  : "Încearcă o perioadă diferită sau o altă destinație"}
              </p>
              {hasFilters ? (
                <button onClick={resetFilters} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl text-sm font-medium">
                  Resetează filtrele
                </button>
              ) : (
                <button onClick={() => router.push("/cruises/search")} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl text-sm font-medium">
                  Caută din nou
                </button>
              )}
            </div>
            ...
            <>
              <div className="flex items-center justify-between mb-4">
                <p className="text-gray-600 text-sm">
                  ...
                  {allFares.length !== filteredFares.length && (
                    <span className="text-gray-400"> din {allFares.length}</span>
                  )}
                  {" "}croaziere găsite
                </p>
                {hasFilters && (
              ...
              <div className="flex flex-col gap-3">
                {filteredFares.map((fare) => (
                  <CruiseCard
                    key={fare.cruiseID}
                    fare={fare}
                    onSelect={() => router.push(`/cruises/${fare.cruiseID}`)}
                  />
                ))}
              </div>
            </>
